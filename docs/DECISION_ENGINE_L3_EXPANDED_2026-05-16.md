...
7. **`/qa` Panel 6 "Shadow Factors"** — Fisher-exact lift vs baseline for `factor_short_squeeze` at 3d horizon, status badge `🟡 ACCRUING` / `🟢 READY` / `🔴 NEGATIVE`, tripwire target 2026-06-15 → **RESET to 2026-07-18** (the data path was broken from ship — Polygon SI/short-vol returned stale rows and the ordinal never reached the harvested slot, so the tripwire read N=0; fixed 2026-06-18, clock restarts). ⚠️ **TRIPWIRE CLOSED — do NOT reset this date again: `factor_short_squeeze` was KILLED 2026-07-05** (weight 0, promotion criteria structurally unmeetable — see `Confluence_Scanner/BUILD_STATE.md`). The 2026-07-18 target above is retained HISTORY, not an open tripwire; annotated 2026-07-27 so the session-start lapsed-marker hook stops firing on a resolved item forever (`feedback_signals_must_be_able_to_go_quiet` — a check that cannot go green teaches people to skip the whole tool). The code rip is tracked on the scanner track.
